...
function mqttTopicAdd(id) {
    var html;
    html =	'<div class = "box-full-sub"><h3>Topic ' + id + '</h3></div>'+
            '<div class="box-topic">' +
                '<div class="box-16">' +
...
function mqttInit() {
    ...
    return new Promise(async(res, rej) => {
        ...
        await mqttCfgGet()
        .then(async() => {await mqttStateGet()})
        .then(async() => {await mqttTopicCfgGet([0,1,2,3])})
        .then(async() => {await mqttTopicCfgGet([4,5,6,7])})
        .then(async() => {await mqttTopicCfgGet([8,9,10,11])})
        .then(async() => {await mqttTopicCfgGet([12,13,14,15])})
        .then(() => {
            res();
        }).catch((m) => {
            rej(m);
